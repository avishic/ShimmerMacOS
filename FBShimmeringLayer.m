...
static CABasicAnimation *shimmer_slide_animation(CFTimeInterval duration, FBShimmerDirection direction)
{
  ...
  animation.toValue = [NSValue valueWithPoint:NSPointFromCGPoint(CGPointZero)];
  animation.duration = duration;
...
- (instancetype)init
{
  ...
  if (nil != self) {
    ...
    _fadeLayer.backgroundColor = [NSColor whiteColor].CGColor;
...
- (void)_updateMaskColors
{
  ...
  NSColor *maskedColor = [NSColor colorWithWhite:1.0 alpha:_shimmeringOpacity];
  NSColor *unmaskedColor = [NSColor colorWithWhite:1.0 alpha:_shimmeringAnimationOpacity];
